...
public class Solution {
    public static void main(String[] args) {
        String sequence1 = "ABCDAFMNK";
        String sequence2 = "ACDBASMNKXX";
        System.out.println(maxCommonSubSequence(sequence1, sequence2));
    }

    private static String maxCommonSubSequence(String sequence1, String sequence2) {
        ...
        for (int i = 1 ; i <= sequence1.length(); i++)
            for (int j = 1 ; j <= sequence2.length(); j++) {
                ...
                if (character1 == character2){
                    ...
                    assignMaxOfPreviousIndexes(maxSubStringEndingInIJ, i, j);
                }
            }
    ...
    private static void assignMaxOfPreviousIndexes(String[][] maxSubStringEndingInIJ, int i, int j) {
        if(maxSubStringEndingInIJ[i-1][j].length() > maxSubStringEndingInIJ[i][j-1].length())
            maxSubStringEndingInIJ[i][j] = maxSubStringEndingInIJ[i-1][j];
        else
            maxSubStringEndingInIJ[i][j] = maxSubStringEndingInIJ[i][j-1];
    }
